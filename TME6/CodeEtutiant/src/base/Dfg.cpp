...
Dfg::Dfg(Basic_block *bb){
  ...
  // A COMPLETER
  // stop et debut
  Instruction *stop, *itmp=bb->get_first_instruction();
  ...
  for(; itmp != stop ; itmp = itmp->get_next()) {
    // noeud
    ...
    // prec
     ...
     for(int i = 0; i<itmp->get_nb_pred(); i++) {
       // delay
       int delay = -1;
       if(i->get_succ_dep(i)->type == WAW) {
	 delay = WAW_DELAY;
      } else if(i->get_succ_dep(i)->type == WAR) {
	delay = WAR_DELAY;
      } else if(i->get_succ_dep(i)->type == MEMDEP) {
	delay = MEMDEP_DELAY;
      }

       // ajout
      if(delay != -1) {
	// TODO trouver le noeud precedant
	???->add_successeur(new Arc_t(delay, i->get_succ_dep(i)->type, node));
      }
    }
  }
}  
